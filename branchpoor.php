	...
	//if(!mysql_select_db($dbname)) die(mysql_error());
		...
		$count = 1;				
			for($k = 0 ; $k< count($allowed_subjects) ;$k++) {
				...
				$poor[$sub][$brcls][$users[$j]]=mysql_fetch_array(mysql_query($que_r))[0]."_".$pr."_".mysql_fetch_array(mysql_query($que_n))[0];
					}
		}	
	}
...
echo <<<table_head
	\n<table class="table table-hover table-bordered" style="padding:0px;">
		<thead>
			<tr>
			<th  style="text-align:center;width:10%;"  > Class </th> <th  style="text-align:center;width:10%;"  > Subject </th><th  style="text-align:center;width:10%;"  > Id </th><th  style="text-align:center;width:10%;"  > RNo</th> <th  style="text-align:center;"  > Name </th><th  style="text-align:center;width:10%;" > Percentage </th>
			</tr>
		</thead>
		<tbody>
table_head;
...
$sub2 = $sub_def[$key];
$pcs++;
foreach($value as $ke2 => $val3 ) {
$brcls = $ke2;
//print_r($val2);
foreach($val3 as $key1 => $value1) {
$det=explode("_",$value1);
$rn=$det[0];
$name = $det[2];
$per=$det[1];
$lnk = "./students.php?".$key1."/sub";
echo  '<tr onclick="document.location.href=\''.$lnk.'\';" style="cursor:pointer;"><td style="text-align:center;">'.$brcls.'</td><td style="text-align:center;">'.$key.'</td><td style="text-align:center;">'.$key1.'</td><td style="text-align:center;" class = "span1">'.$rn.'</td><td class = "span3">'.$name.'</td><td style="text-align:center;" class="text-error">'.$per.'%</td></tr>';
$tr =   '<tr ><td style="text-align:center;">'.$brcls.'</td><td style="text-align:center;">'.$key.'</td><td style="text-align:center;">'.$key1.'</td><td style="text-align:center;" class = "span1">'.$rn.'</td><td class = "span3" style="text-align:left;">'.$name.'</td><td style="text-align:center;" ><font color=darkred>'.$per.'%</font></td></tr>';
...
$pct++;
	...
	if($row['Position']=="BA"){
		...
		<input type='hidden' name='Title1' value="$branch - Poor Attendance Report">
